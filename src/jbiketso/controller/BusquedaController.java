...
public class BusquedaController extends Controller implements Initializable {
    ...
    private void buscarSedes() {
        ...
        Resultado<ArrayList<BikSede>> sedes = FuncionarioDao.getInstance().getSedeFiltro(filtros.get("nombre").toString());
        if (sedes.getResultado().equals(TipoResultado.SUCCESS)) {
            tbvResultados.setItems(FXCollections.observableArrayList(sedes.get()));
            tbvResultados.refresh();
        }
    }

    private void setBuscaSedesListener(TextField textfield) {
        textfield.setOnKeyPressed((KeyEvent ke) -> {
            if (ke.getCode().equals(KeyCode.ENTER)) {
                filtros.put(textfield.getId(), textfield.getText());
                buscarSedes();
