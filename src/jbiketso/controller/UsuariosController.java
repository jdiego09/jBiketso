...
public class UsuariosController extends Controller implements Initializable {
    ...
    private void init() {
        ...
        this.estados.add(new GenValorCombo("F", "Fallecido"));
    ...
    void imprimirContrato(ActionEvent event) {
        if (this.expediente.getExpCodigo() != null && this.expediente.getExpCodigo() > 0) {
            ...
            Aplicacion.getInstance().generarReporte("rpt_contrato", parametros);
        } else {
    ...
    void imprimirExpediente(ActionEvent event) {
        if (this.expediente.getExpCodigo() != null && this.expediente.getExpCodigo() > 0) {
            HashMap<String, Object> parametros = new HashMap<>();
            parametros.put("codigoExpediente", this.expediente.getExpCodigo());
            Aplicacion.getInstance().generarReporte("rpt_expediente", parametros);
        } else {
            AppWindowController.getInstance().mensaje(Alert.AlertType.WARNING, "Imprimir expediente", "No se ha consultado ningún expediente para imprimir.");
        }
    }
